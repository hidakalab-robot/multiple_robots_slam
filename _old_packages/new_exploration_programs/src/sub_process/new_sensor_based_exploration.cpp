...
const int loop_closing_max = 4;//プログラムを切り替えるために必要なループクロージングの回数
...
void Branch_search(std::vector<float> &fixed_ranges,std::vector<float> &fixed_angle,float max_angle){
	...
	const float branch_y_low = 0;
	const float branch_y_high = 100;//分岐領域の2点間のｙ座標の差がこの値以下のとき分岐として検出
...
void VFH4vel_publish_Branch(){
	...
	const float gra_enable = std::abs(goal_y)+1.8;
...
int main(int argc, char** argv){
	...
	tf_option = ros::SubscribeOptions::create<geometry_msgs::Point>("/loop_counter",1,tf_callback,ros::VoidPtr(),&tf_queue);
	tf_sub = s.subscribe(tf_option);
