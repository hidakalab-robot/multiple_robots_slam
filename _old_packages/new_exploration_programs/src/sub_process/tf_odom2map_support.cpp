...
int main(int argc, char** argv){
	...
	ros::init(argc, argv, "tf_odom2map_support");
	ros::NodeHandle tos;

	os_pub = tos.advertise<geometry_msgs::Point>("/odom2map_support", 1);


	odom_option = ros::SubscribeOptions::create<geometry_msgs::Point>("/odom2map",1,odom_callback,ros::VoidPtr(),&odom_queue);
	odom_sub = tos.subscribe(odom_option);
