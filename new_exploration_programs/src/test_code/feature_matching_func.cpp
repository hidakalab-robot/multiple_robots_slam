...
orig_master_cloud_for_shift(new pcl::PointCloud<pcl::PointXYZRGB>)
{
  ...
  smc.setCallbackQueue(&mc_queue);
 	sc_sub = ssc.subscribe("/pointcloud_segmentation/source_cloud",1,&FeatureMatching::input_sourcecloud,this);
  mc_sub = smc.subscribe("/pointcloud_segmentation/source_cloud",1,&FeatureMatching::input_mastercloud,this);
  input_master = false;
  input_source = false;
  changed_master = false;
  matching_threshold = 0.17;
  ...
  origm_pub = prc.advertise<sensor_msgs::PointCloud2>("pointcloud_segmentation_registered/orig_master_cloud", 1);
  origs_pub = prc.advertise<sensor_msgs::PointCloud2>("pointcloud_segmentation_registered/orig_source_cloud", 1);
};
