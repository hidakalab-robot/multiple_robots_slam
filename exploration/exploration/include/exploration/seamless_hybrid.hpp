...
private:
    ...
    struct areaInfo{ //これのサイズ : 分岐の数 + 1(直進) + 自分以外のロボットの数 = 分岐の数 + ロボットの数
        ...
        areaInfo(){};
        areaInfo(const std::string& n,const geometry_msgs::Point& p,const Eigen::Vector2d& v):robot(n,p,v[0],v[1]){};
    };
    ...
    std::vector<robotInfo> robotList; // 外部から取得する, 全ロボットのポーズ
...
bool SeamlessHybrid::dataFilterMulti(void){
    ...
    //ロボットリストのフィルタ
    auto eraseRobotIndex = std::remove_if(robotList.begin(), robotList.end(),[this](SeamlessHybrid::robotInfo& r){return r.name == ROBOT_NAME;});
    robotList.erase(eraseRobotIndex,robotList.end());
...
void SeamlessHybrid::evaluationInitializeMulti(void){
...
bool SeamlessHybrid::resultMulti(geometry_msgs::Point& goal){
