...
void VoronoiPlanner::initialize(std::string name, costmap_2d::Costmap2D* costmap, std::string frame_id) {
    ...
    if (!initialized_) {
        ...
        static ros::Subscriber costmapUpdateSubscriber = private_nh.subscribe("/move_base/global_costmap/costmap_updates", 10, &VoronoiPlanner::costmapUpdateCallback, this);
        initialized_ = true;
    } else
...
bool VoronoiPlanner::makePlan(const geometry_msgs::PoseStamped& start, const geometry_msgs::PoseStamped& goal,
                           std::vector<geometry_msgs::PoseStamped>& plan, nav_msgs::OccupancyGrid& map) {
    return makePlan(start, goal, default_tolerance_, plan, map);
}
...
bool VoronoiPlanner::makePlan(const geometry_msgs::PoseStamped& start, const geometry_msgs::PoseStamped& goal,
                           double tolerance, std::vector<geometry_msgs::PoseStamped>& plan, nav_msgs::OccupancyGrid& emptyMap) {
    boost::mutex::scoped_lock lock(mutex_);
    if (!initialized_) {
        ROS_ERROR(
                "This planner has not been initialized yet, but it is being used, please call initialize() before use");
...
void VoronoiPlanner::publishVoronoiGrid(DynamicVoronoi *voronoi,nav_msgs::OccupancyGrid& map)
{
    int nx = costmap_->getSizeInCellsX(), ny = costmap_->getSizeInCellsY();
