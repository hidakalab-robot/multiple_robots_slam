...
int main(int argc, char* argv[]){
    ...
    ros::Publisher pub = ros::NodeHandle().advertise<geometry_msgs::PoseArray>("canceled_goals",1,true);
    std::vector<move_base_msgs::MoveBaseActionGoal> goals;
    geometry_msgs::PoseArray canceledGoals;
        ...
        if((msg->status.status==2&&msg->status.text=="")|| msg->status.status > 3){
            for(int i=goals.size()-1;i!=0;--i){
                if(msg->status.goal_id.id== goals[i].goal_id.id){
                    canceledGoals.poses.emplace_back(goals[i].goal.target_pose.pose);
